...
export async function sendInquiryConfirmationEmail(
  ...
  if (operation === "登録車両変更" && details && details.newCarModel && details.newCarColor) {
    ...
    // 解約理由がある場合は黄色背景で目立たせる
    if (details.withdrawalReason && details.withdrawalReason.length > 0) {
      detailsHtml += `
        <div style="margin: 15px 0; padding: 15px; background-color: #fff3cd; border-left: 4px solid #ffc107; border-radius: 4px;">
          <p style="margin: 0 0 10px; font-size: 15px; font-weight: 600; color: #856404;">✓ 選択された解約理由:</p>
          <ul style="margin: 0; padding-left: 20px; color: #856404;">
            ${details.withdrawalReason.map((reason: string) => `<li style="margin: 5px 0;">${reason}</li>`).join("")}
          </ul>
        </div>
      `
    }
        ...
        <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px; margin: auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
          ...
          <tr>
            <td style="padding: 30px 30px 20px;">
              <p style="font-size: 16px; margin-top: 0; margin-bottom: 20px; font-weight: 500;">${customerName} 様</p>
